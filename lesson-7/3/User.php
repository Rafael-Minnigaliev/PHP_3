...
class User
{
    ...
    {
        $key = $this->getGlobalKey($user['login'], $user['id']);
        $this->identityMap[$key] = $user;
    }
    public function get($userLogin, int $userId)
    {
        $key = $this->getGlobalKey($userLogin, $userId);
        if (isset($this->identityMap[$key])) {
            return $this->identityMap[$key];
        }
        throw new EmptyCacheException();
    }
    private function getGlobalKey(string $userLogin, int $userId)
    {
        return sprintf('%s.%d', $userLogin, $userId);
    }
    ...
    public function getById(int $id): ?Entity\User
    {
        foreach ($this->getDataFromSource(['id' => $id]) as $user) {
            if ($this->get($user['login'], $user['id'])){
                return $this->get($user['login'], $user['id']);
            }else{
                $crUser = $this->createUser($user);
                $this->add($crUser);
                return $crUser;
            }
        }
